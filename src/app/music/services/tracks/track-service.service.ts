...
import { ElectronService } from 'ngx-electron';//I wrote ngx-electron a small package, which wraps all the electron API’s exposed to the renderer process into a single ElectronService
...
export class TrackService {
  ...
  constructor(private _electronService: ElectronService, private appRef: ApplicationRef) {
    ...
    ipc.on('mp3-file', (event, arg) => {
      ...
      let notification = new Notification('music player', {
  ...
  public play = () => {
    if (!this.trackList.length) {
      ...
      let notification = new Notification('sound', {
